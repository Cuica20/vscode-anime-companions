...
export abstract class BasePokemonType implements IPokemonType {
  ...
  recoverState(state: PokemonInstanceState) {
    ...
    if (!isStateAboveGround(this.currentStateEnum)) {
      // Reset the bottom of the sprite to the panel floor.
  ...
  private getAnimationSrc(
    face: string,
    hasLeftFacingSprite: boolean | undefined,
  ): string {
    const builtInFace =
      !hasLeftFacingSprite && face === 'walk_left' ? 'walk' : face;

    if (!this._customAnimations) {
      return `${this.pokemonRoot}_${builtInFace}_8fps.gif`;
    }

    return (
      this._customAnimations[face] ??
      this._customAnimations[builtInFace] ??
      (face.includes('walk')
        ? this._customAnimations.walk
        : this._customAnimations.idle)
    );
  }
